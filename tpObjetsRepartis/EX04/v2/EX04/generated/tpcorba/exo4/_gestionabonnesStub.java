	...
	implements tpcorba.exo4.gestionabonnes
{
	...
	public tpcorba.exo4.abonne[] liste_abonnes()
	{
		while(true)
		{
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request("_get_liste_abonnes",true);
				_is = _invoke(_os);
				return tpcorba.exo4.table_abonneHelper.read(_is);
			}
			catch( org.omg.CORBA.portable.RemarshalException _rx ){}
			catch( org.omg.CORBA.portable.ApplicationException _ax )
			{
		...
		{
		org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "_get_liste_abonnes", _opsClass);
		if( _so == null )
			throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			gestionabonnesOperations _localServant = (gestionabonnesOperations)_so.servant;
		...
		try
		{
			_result = _localServant.liste_abonnes();
		}
		finally
		{
			_servant_postinvoke(_so);
	...
	public void creation_abonne(int numero, java.lang.String nom, java.lang.String prenom, tpcorba.exo4.type_adresse adresse, tpcorba.exo4.type_abonnement abonnement, tpcorba.exo4.abonneHolder ref) throws tpcorba.exo4.dejaExistant
	{
		while(true)
		{
		if(! this._is_local())
		{
			...
			try
			{
				...
				tpcorba.exo4.type_adresseHelper.write(_os,adresse);
				tpcorba.exo4.type_abonnementHelper.write(_os,abonnement);
				tpcorba.exo4.abonneHelper.write(_os,ref.value);
				_is = _invoke(_os);
				ref.value = tpcorba.exo4.abonneHelper.read(_is);
				return;
		...
		else
		{
			...
			try
			{
			_localServant.creation_abonne(numero,nom,prenom,adresse,abonnement,ref);
			}
			finally
			{
				_servant_postinvoke(_so);
