	...
	implements tpcorba.exo3.compte
{
	...
	public java.lang.String afficher()
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request("_get_afficher",true);
				_is = _invoke(_os);
		...
		{
		org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "_get_afficher", _opsClass);
		if( _so == null )
			throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			compteOperations _localServant = (compteOperations)_so.servant;
		...
		try
		{
			_result = _localServant.afficher();
		}
		finally
		{
			_servant_postinvoke(_so);
